...
```
pentest-intake (legitimacy gate + config)  →  scope-analyst  →  rules.yaml  (the law, injected to all)
                                                                       │
   ┌──────────────────── Super-Agent (Principal) drives the loop ──────┴────────────┐
   │  Recon workflow                         Attack workflow                         │
   │  N tool-agents (1 per allowed tool)     N tool-agents                           │
   │     → ceil(N/5) orchestrators           → ceil(N/5) orchestrators               │
   │     → master-recon                      → master-attack                         │
   │                                         → persistence-controller (retry)        │
   └───────────────────────────────────────────────────────────────────────────────┘
   Creative pool (firewalled, no memory) → recon/attack → Super-Agent (Global) correlates
   Learning loop: any agent → inbox → skill-writer → learned skills (base runs stay immutable)
```
